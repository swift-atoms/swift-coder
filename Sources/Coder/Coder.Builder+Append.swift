...
extension Coder.Builder where Input: ~Copyable & ~Escapable, Buffer: ~Copyable & ~Escapable {
    ...
    public static func buildPartialBlock<A: Coder.`Protocol`, N: Coder.`Protocol`, each O>(
        accumulated: A,
        next: N
    ) -> Coder.Append<A, N, Either<A.Failure, N.Failure>, repeat each O>
    where
        A.Input == Input,
        N.Input == Input,
        A.Buffer == Buffer,
    ...
    {
        Coder.Append(accumulated, next, { .left($0) }, { .right($0) })
    }
    ...
    public static func buildPartialBlock<A: Coder.`Protocol`, N: Coder.`Protocol`, each O>(
        accumulated: A,
        next: N
    ) -> Coder.Append<A, N, A.Failure, repeat each O>
    where
        A.Input == Input,
        N.Input == Input,
        A.Buffer == Buffer,
        ...
        A.Failure == N.Failure
    {
        Coder.Append(accumulated, next, { $0 }, { $0 })
    }
}
