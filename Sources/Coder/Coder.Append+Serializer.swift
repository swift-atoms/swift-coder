...
extension Coder.Append: Serializer.`Protocol`
where
    ...
    N.Buffer: ~Copyable & ~Escapable
{
    ...
    public borrowing func serialize(_ output: borrowing Output, into buffer: inout Buffer) throws(Failure) {
        ...
        do throws(A.Failure) {
            try body.accumulated.serialize(head, into: &buffer)
        } catch {
            throw body.accumulatedFailure(error)
        }
        do throws(N.Failure) {
            try body.next.serialize(last, into: &buffer)
        } catch {
            throw body.nextFailure(error)
        }
    }
}

extension Coder.Append: Coder.`Protocol`
where
    A: Coder.`Protocol`,
    N: Coder.`Protocol`,
    A.Buffer == N.Buffer,
